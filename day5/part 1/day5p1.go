...
func main() {
	...
	for scanner.Scan() {
		...
		if stackingInitialCrates {
			...
			for i := 0; i < moveCrateCount; i++ {
				crateStacks[moveSrcDest] = prependString(crateStacks[moveSrcDest], crateStacks[moveSrcColumn][0])
				crateStacks[moveSrcColumn] = removeIndex(crateStacks[moveSrcColumn], 0)
			}
		}
	}
